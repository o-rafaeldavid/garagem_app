...
class _LandingScreenState extends State<LandingScreen> with RouteAware {
  ...
  void initState() {
    ...
    mqtt.onSuccessCamera = (String payload) {
      if (_landingTOCameraInicializado) {
        Navigator.pushNamed(context, "/camera", arguments: payload);
      }
    };
  ...
  Future<void> _goMaps({String? query = ""}) async {
    ...
    Uri uri =
        Uri(scheme: 'https', host: 'www.google.com', path: 'maps/$search');
    if (!await launchUrl(
      uri,
  ...
  Widget build(BuildContext context) {
    ...
    double half_widthGapped =
        (MediaQuery.of(context).size.width - 5 * GlobalVars.gap) * 0.5;
    double quarter_widthGapped =
        (MediaQuery.of(context).size.width - 5 * GlobalVars.gap) * 0.25;
    const double columnGap = 0.25 * GlobalVars.gap;

    List<List<Widget>> bentoBox(GarageStatus gStatus) {
      _reservedTime = gStatus.createdAt;
      return [
        <Widget>[GarageMiniInfo(gStatus: gStatus)],
        <Widget>[
          RoundRectColumned(
              onTap: () {
                _goMaps(query: gStatus.title);
              },
              ...
              children: <Widget>[
                Text("Check Garage on Maps",
                    style: GoogleFonts.orbitron(
                        textStyle:
                            TxtStyles.paragraph(AllCores.laranja(255), 6)))
              ]),
        ],
        <Widget>[
          RoundRectColumned(
              ...
              children: <Widget>[
                Text("Status",
                    style: GoogleFonts.orbitron(
                        textStyle:
                            TxtStyles.heading2(AllCores.laranja(255), 8))),
                const SizedBox(
                  height: columnGap,
                ),
                if (gStatus.porta_estado == "Open" ||
                    gStatus.porta_estado == "Opening")
                  Row(children: <Widget>[
                    Text(gStatus.porta_estado,
                        style: GoogleFonts.orbitron(
                            color: AllCores.amarelo(255),
                            textStyle: TxtStyles.paragraph(null, 0))),
                    const SizedBox(width: columnGap * 0.7),
                    const Icon(Icons.warning_rounded,
                        size: 0.53 * GlobalVars.iconSize, color: Colors.white)
                  ])
                else
                  Text(gStatus.porta_estado,
                      style: GoogleFonts.orbitron(
                          color: AllCores.branco(128),
                          textStyle: TxtStyles.paragraph(null, 0)))
              ]),
          RoundRectColumned(
              onTap: () {
              ...
              children: const <Widget>[
                Icon(Icons.close_rounded,
                    size: 1.3 * GlobalVars.iconSize, color: Colors.white)
              ]),
        ],
        <Widget>[
          RoundRectColumned(
              onTap: () {
                if (gStatus.porta_estado != "Opening" ||
                    gStatus.porta_estado != "Closing") {
                  if (gStatus.porta_estado == "Closed") {
                    mqtt.sendMessage("abre-te sesamo", Topics.publish[1]);
                  } else {
                    mqtt.sendMessage("fecha-te chia", Topics.publish[1]);
                  }
                }
              },
              color: (gStatus.porta_estado == "Open")
                  ? AllCores.vermelho(50)
                  : (gStatus.porta_estado == "Closed")
                      ? AllCores.verde(25)
                      : AllCores.amarelo(25),
              ...
              children: <Widget>[
                Expanded(
                    child: (gStatus.porta_estado != "Opening" &&
                            gStatus.porta_estado != "Closing")
                        ? (gStatus.porta_estado == "Open")
                            ? Text("Close\nGarage",
                                style: GoogleFonts.orbitron(
                                    textStyle: TxtStyles.heading2(
                                        AllCores.vermelho(255), 8)))
                            : Text("Open\nGarage",
                                style: GoogleFonts.orbitron(
                                    textStyle: TxtStyles.heading2(
                                        AllCores.verde(255), 8)))
                        : Text(gStatus.porta_estado,
                            style: GoogleFonts.orbitron(
                                textStyle: TxtStyles.heading2(
                                    AllCores.amarelo(255), 8)))),
                Container(
                    alignment: Alignment.bottomRight,
                    height: GlobalVars.iconSize,
                    child: Image.asset(
                        'lib/assets/icons/lock_${(gStatus.porta_estado == "Open" || gStatus.porta_estado == "Opening") ? "" : "un"}locked.png'))
              ]),
          RoundRectColumned(
              onTap: () {
                setState(() {
        ...
        <Widget>[
          Expanded(
              child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: <Widget>[
                Text("has been reserved for",
                    textAlign: TextAlign.center,
                    style: GoogleFonts.orbitron(
                        textStyle: TxtStyles.paragraph(null, 0))),
                Text(_reservedString,
                    textAlign: TextAlign.center,
                    style: GoogleFonts.orbitron(
                        textStyle:
                            TxtStyles.heading2(AllCores.laranja(255), 8))),
              ...
              ? (_lastRow != null && _lastRow!.active)
                  ? Container(
                      ...
                      child: SingleChildScrollView(
                        ...
                        return Container(
                            margin:
                                const EdgeInsets.only(bottom: GlobalVars.gap),
                            child: Row(
                                mainAxisAlignment:
                          ...
                          WarningInfo(
                            ...
                            widgets: <Widget>[
                              RoundRect(
                                  onTap: () {
                                    Navigator.pushNamed(
                                        context, NavigationHelper.routes[1]);
                                  },
                                  child: Text("Scan QR Code",
                                      style: GoogleFonts.orbitron(
                                          textStyle:
                                              TxtStyles.paragraph(null, 0))))
                            ],
                          )
                        ]))
              : Center(
                  child: Container(
                      padding: const EdgeInsets.only(
                          bottom: GlobalVars.appbarHeight),
                      child: SizedBox(
                          height: 2 * GlobalVars.iconSize,
                          child: Image.asset('lib/assets/gifs/loading.gif')))),
